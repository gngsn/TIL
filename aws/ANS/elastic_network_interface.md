...
- Secondary IP 사용 가능 범위에 대해 더 자세히 알아보기
- NLB 는 IP가 변경될 일이 없지만, ALB는 운영 중일 때도 IP가 변경될 수 있음
- **AWS EC2 - ENIs**: 3개의 Primary IPs 들과 각 Pods에 할당되는 Secondary IPs 들을 확인할 수 있음
- AWS EC2 별로 ENI 를 선택하는데 각 옵션 별로 네트워크 성능(대역폭)이 정해져있음
