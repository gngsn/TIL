...
<details>
<summary><h4>TL;DR</h4></summary>

- **모니터링의 중요성**: 안정적인 서비스 운영을 위해 필수적이며, 서비스 변경 영향과 장애 발생 시 논리적 대응이 가능하게 함.
- **초대형 시스템 모니터링의 어려움**: 컴포넌트 수 파악, 엔지니어 책임 부담 최소화, 종합적 신호 전달의 필요성 등.
- **Borgmon**: 구글에서 개발한 시계열 모니터링 도구
    ...
    - **모니터링 토폴로지의 샤딩**: 스트리밍 프로토콜을 사용하여 CPU 시간을 절약하고, 데이터 전송 크기를 줄임.
- **Borgmon**: Borgmon은 낮은 오버헤드로 대량 데이터를 수집하고, 공용 데이터 해설 형식을 사용하여 시스템 장애를 탐지함.
  - **데이터 수집 및 저장**
    - Borgmon은 메모리에 데이터를 저장하고, 정기적으로 디스크에 저장하며, 시계열 데이터베이스(TSDB)를 통해 더 많은 데이터를 보관함.
    - `varz`: 내부 상태를 내보내는 메서드로, 평문 형태의 변수 값을 출력하며, 다양한 지표를 제공함.
- **레이블과 벡터**: 시계열 데이터는 숫자 시퀀스와 타임스탬프 값으로 저장되며, 레이블셋을 통해 쿼리 가능함.
- 보그몬 규칙: 시계열 데이터를 계산하고 알림을 생성하는 규칙을 정의하여 병렬 스레드 풀에서 실행 가능.
- **Alertmanager**: 알림 시스템 - 알림 규칙을 평가하여 조건이 충족될 때만 알림을 발송하며, 중앙 집중식 알림 매니저(Alertmanager)와 연동됨.
- **레이블**: 시계열 데이터를 그룹화하고 집계하는 데 사용되며, 데이터의 원본과 집계 지점을 정의함.
...
`varz` HTTP 핸들러는 내보내는 변수 값들을 평문 형태로 출력하며, 키와 값은 한 줄에 공백으로 구분됨
