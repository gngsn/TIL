...
1. **낮은 추상화**: 고수준 추상화.
  - 예시: 펄(Perl)
  - 특징: POSIX 수준의 행동 유사성을 제공하는 범용 언어들은 이론적으로 시스템이 접근할 수 있는 API를 통해 거의 제약 없는 자동화가 가능함.
  - 구글 프로덕션 환경의 높은 복잡도로 인해 경우에 따라 사태를 즉시 추적하고 파악하기에 더 적합한 경우가 존재.

2. **높은 추상화**: 저수준 추상화. 서비스나 좀 더 높은 수준의 엔티티들에 대한 조작을 통해 한 단계 높은 추상화를 제공.
  - 예시: 퍼펫(Puppet), 셰프(Chef)
  - 장점: 관리하고 추론하기가 더 쉬움.
  - 단점: 시스템적으로, 반복적으로, 그리고 잠재적 불일치로 인한 실패가 발생할 수 있는 '추상화의 누수' (leaky abstraction)를 경험할 수도 있음.

<br>

### 자동화 클래스의 계층 구조

이상적인 경우라면 자동화를 위한 별도의 시스템으로 구현할 필요는 없음.

시스템을 구축할 때는 외부의 다른 시스템과 통합하기 위한 로직은 배제하는 편이 더 나음. (애초에 자동화 시스템 자체가 외부 시스템과의 통합 로직이 필요하지 않도록 고안되었기 때문.)
...
구글의 경험상, 자동화나 자율적 동작을 필요로 하는 시스템은 계속해서 늘어나고 있음. 규모가 커지는 것도 마찬가지이지만, 규모에 관계없이 스케줄링 없이 시스템에 자율성을 추가하는 것에 대해서는 여전히 논란이 있음. 

신뢰성은 근본적인 기능임. 

그러므로 자율적이면서 탄력적으로 구현되었을 때 그 빛을 발함.
