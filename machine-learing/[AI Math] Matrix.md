...
**1번째 행벡터의 1번째 원소 (`R[0][0]`)  `-8` 이 어떻게 계산되었을까**

$X$ 의 첫 번째 행벡터(`X[0][:]`) 와 $Y$ 의 첫 번째 열벡터(`Y[:][0]`)의 내적

 `1・0 + (-2)・1 + 3・(-2) = -8` 으로 계산됨



**1번째 행벡터의 2번째 원소 (`R[0][1]`) `6` 이 어떻게 계산되었을까**

$X$ 의 첫 번째 행벡터(`X[0][:]`) 와 $Y$ 의 두 번째 열벡터(`X[1][:]`) 의 내적

 `1・1 + (-2)・(-1) + 3・1 = 6` 으로 계산됨
